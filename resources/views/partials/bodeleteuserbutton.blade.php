<div x-data="{ open: {{ isset($open) && $open ? 'true' : 'false' }}, working: false }" x-cloak wire:key="del_{{ $value }}">
    <span x-on:click="open = true">
        <button class="p-1 text-alert rounded hover:bg-alert hover:text-white" title="{{ __('Delete') }}"><x-icons.trash /></button>
    </span>
    ...
    <div x-show="open"
        ...
        <div x-show="open" x-transition:enter="ease-out duration-300" x-transition:enter-start="opacity-0"
            ...
            <div class="absolute inset-0 bg-primary-400 opacity-75"></div>
        </div>
        ...
        <div x-show="open" x-transition:enter="ease-out duration-300"
            ...
            <div class="hidden sm:block absolute top-0 right-0 pt-4 pr-4">
                <button x-on:click="open = false" type="button"
                    class="text-primary-300 hover:text-primary-400 focus:outline-none focus:text-primary-400 transition ease-in-out duration-150">
